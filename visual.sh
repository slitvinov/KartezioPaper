...
do d=${d%%/}
   ...
   do p=${i%%.gv}.ppm
      dot "$i" -o /tmp/visual.png  -T png -Gsize=9,15\! -Gdpi=100
      convert /tmp/visual.png -gravity center -background white -extent 900x1500 $p
      if test -f "$pp"
   ...
   convert $d/00000000.ppm -bordercolor red -border 10x10 \
	   -gravity northeast -fill black -pointsize 100 -annotate +10+10 `printf %02d $g` \
	   $d/00000000.ppm
