    ...
    for color_scale in COLORS_SCALES:
        ...
        for y_true in dataset.test_y:
            ...
            for i in range(1, n_annotations + 1):
                roi_pixel_areas.append(
                    np.count_nonzero(annotations[annotations == i]))
        ...
        for i, model in enumerate(pool.models):
            ...
            p_test, fitness, t = model.eval(dataset,
                                            subset="test",
            ...
            for j, p_test_i in enumerate(p_test):
                ...
                if "mask_raw" in p_test_i:
                    cm_mask = cv2.applyColorMap(
                        (image_normalize(p_test_i["mask_raw"]) * 255).astype(
                            np.uint8),
                        cv2.COLORMAP_VIRIDIS,
                    )
                else:
                    cm_mask = cv2.applyColorMap(
                        (image_normalize(p_test_i["mask"]) * 255).astype(
                            np.uint8),
                        cv2.COLORMAP_VIRIDIS,
                    )

            # Training set
            p_train, fitness, _ = model.eval(dataset,
                                             subset="train",
                                             preprocessing=preprocessing)
        ...
        print_stats(scores_training, "AP50",
